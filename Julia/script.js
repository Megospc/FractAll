...
void main() {
  ...
  ar = v_coord.x*u_scale+u_center_x;
  ai = v_coord.y*u_scale*${scrx}+u_center_y;
  ...
  const xloc = gl.getUniformLocation(program, 'u_x');
  ...
  draw = function() {
    ...
    gl.uniform1f(xloc, cr.value/100);
    gl.uniform1f(yloc, ci.value/100);
    ...
    crl.innerHTML = (cr.value/100).toFixed(2);
    cil.innerHTML = (ci.value/100).toFixed(2);
  };
  ...
  const Complex = double ? class {
    ...
    is() {
      return this.r.mul(this.r).add(this.i.mul(this.i)).gt(max);
    }
  }:class {
    constructor(r, i) {
      this.r = r;
  ...
  draw = function() {
    const pr = cr.value/100;
    const pi = ci.value/100;
    const zr = double ? doub(pr):pr;
    const zi = double ? doub(pi):pi;
    ...
    function f(id, ix = 0, iy = 0, iw = iscale, ih = iscale) {
      ...
      for (let x = 0; x < width; x += iscale) for (let y = 0; y < height; y += iscale) {
        ...
        let a = new Complex(cr, ci);
        
        const c = new Complex(zr, zi);
